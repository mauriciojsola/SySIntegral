...
namespace SySIntegral.Web.Areas.Admin.Controllers
{
    ...
    public class CheckPointsController : SySIntegralBaseController
    {
        ...
        public CheckPointsController(IOrganizationRepository organizationRepository, 
        ...
        {
             var checkPoints = IsLimitedByOrganization
                ? _checkPointRepository.GetAll().Where(x => x.Asset.Organization.Id == OrganizationId).Include(x => x.Asset).ThenInclude(x => x.Organization)
                    .OrderBy(x => x.Asset.Organization.Name).ThenBy(x => x.Asset.Name).ThenBy(x => x.CheckPointType).ToList()
                : _checkPointRepository.GetAll().Include(x => x.Asset).ThenInclude(x => x.Organization)
                    .OrderBy(x => x.Asset.Organization.Name).ThenBy(x => x.Asset.Name).ThenBy(x => x.CheckPointType).ToList();
